...
async fn run_thread<E: EngineLoop>(core_id: u8, queue: Box<dyn WorkQueue>) {
    // set core affinity
    ...
    loop {
        ...
        let (args, debt) = match queue.try_get_engine_args() {
            Some(v) => v,
            None => {
...
pub fn start_thread<E: EngineLoop + 'static + std::marker::Send>(cpu_slot: u8, queue: Box<dyn WorkQueue + Send>) -> () {
    // spawn(move || run_thread::<E>(cpu_slot, queue));
    tokio::spawn(run_thread::<E>(cpu_slot, queue));
}
