...
fn unikernel_run_static(
    ...
    //     }
    // }
    dbg!("run unikernel engine static");
    return Ok(())
}

struct UnikernelLoop {
    cpu_slot: u8,
}

impl EngineLoop for UnikernelLoop {
    fn init(core_id: u8) -> DandelionResult<Box<Self>> {
        return Ok(Box::new(UnikernelLoop { cpu_slot: core_id }));
    }
    fn run(
        &mut self,
        ...
        let malloc_context = match &context.context {
            ContextType::Malloc(malloc_context) => malloc_context,
            _ => return Err(DandelionError::ContextMissmatch),
        };

        // let offset = malloc_context.storage.offset();
        let size = malloc_context.layout.size();
        unikernel_run_static(
            self.cpu_slot,
            // malloc_context.storage.filename().unwrap(),
            // offset,
            size,
...
pub struct UnikernelDriver {}

impl Driver for UnikernelDriver {
    ...
    fn start_engine(
        ...
        // }
        dbg!("start Unikernel Driver");
        start_thread::<UnikernelLoop>(0, queue);
        return Ok(()); 
    }
