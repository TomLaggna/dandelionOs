...
pub fn patch_k2u(
    ...
    // kernel_rsp_save at offset 0 (filled at runtime)
    // user_cr3_value at offset 8
    let cr3_offset = data_base + 8;
    storage[cr3_offset..cr3_offset + 8].copy_from_slice(&user_cr3.to_le_bytes());

    // gdt_desc at offset 16 (2-byte limit + 8-byte base)
    let gdt_off = data_base + 16;
    storage[gdt_off..gdt_off + 2].copy_from_slice(&gdt_limit.to_le_bytes());
    storage[gdt_off + 2..gdt_off + 10].copy_from_slice(&gdt_base.to_le_bytes());

    // idt_desc at offset 32 (2-byte limit + 8-byte base)
    let idt_off = data_base + 32;
    storage[idt_off..idt_off + 2].copy_from_slice(&idt_limit.to_le_bytes());
    storage[idt_off + 2..idt_off + 10].copy_from_slice(&idt_base.to_le_bytes());

    // tss_selector at offset 48
    let tss_off = data_base + 48;
    storage[tss_off..tss_off + 2].copy_from_slice(&tss_selector.to_le_bytes());

    // user_stack_top at offset 56
    let stack_off = data_base + 56;
    storage[stack_off..stack_off + 8].copy_from_slice(&user_stack_top.to_le_bytes());

    // user_entry_point at offset 64
    let entry_off = data_base + 64;
    storage[entry_off..entry_off + 8].copy_from_slice(&user_entry_point.to_le_bytes());
}
...
pub fn patch_u2k(storage: &mut [u8], u2k_offset: usize, kernel_cr3: u64) {
    ...
    // kernel_rsp_restore at offset 8 is filled by K2U at runtime
}

/// Get address of K2U trampoline entry point (for calling from kernel)
#[cfg(target_arch = "x86_64")]
pub fn get_k2u_entry() -> u64 {
    unsafe { &port_trampoline_k2u as *const u8 as u64 }
}
